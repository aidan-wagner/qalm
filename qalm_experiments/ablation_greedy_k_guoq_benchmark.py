"""
Greedy_k ablation on the full 250-circuit nam_rz benchmark set.

Same experiment as ablation_greedy_k.py but with 250 circuits from
~/guoq/benchmarks/nam_rz/ instead of the 26 nam_circs.

Results go to greedy_k_guoq_benchmark_results/ (separate from the 26-circuit results).

Run from repo root:
    python3 qalm_experiments/ablation_greedy_k_guoq_benchmark.py
"""
...
import ablation_greedy_k as _exp

_exp.CIRCUIT_LIST = [
    (path, os.path.splitext(os.path.basename(path))[0])
    for path in _qasm_files
]
_exp.RESULTS_DIR = "greedy_k_guoq_benchmark_results"
