"""
N_pool and N_branch comparison experiments (3600s, all 28 Nam circuits).

Phase 1 — N_pool:   vary N_pool 1..5, fix N_branch=1
Phase 2 — N_branch: vary N_branch 1..5, fix N_pool=1
Common:  greedy_k=2 with advancing k (k=3,4,5,...), timeout=3600s,
         32 workers, 8 GB per worker.

Plots exclude hwb6 and ham15-low (26 circuits).
Two aggregate figures per phase: arithmetic mean and geometric mean reduction.
Optimized QASM circuits are stored in the qasm/ subdirectory.

Run from repo root:
    PYTHONPATH=qalm_experiments python3 qalm_experiments/npool_nbranch_experiment.py
"""
...
NPOOL_OUT_DIR = "npool_comparison_3600s_advk_results"
NBRANCH_OUT_DIR = "nbranch_comparison_3600s_advk_results"
...
def _set_mem_limit():
    """Set per-process virtual-memory limit (inherited by subprocesses)."""
    resource.setrlimit(resource.RLIMIT_AS, (MEM_LIMIT_BYTES, MEM_LIMIT_BYTES))


def _parse_output(stdout, circuit_name):
    """Parse progress lines and final QASM from test_qalm stdout."""
    times, costs = [], []
    ...
    for line in stdout.splitlines():
        ...
        if words[0] == prefix:
            try:
                ...
                continue
            # Greedy format has "CX:" at words[4], time at words[10]
...
def run_qalm(filename, circuit_name, timeout, n_pool, n_branch):
    """Run test_greedy_k_ablation with advancing k (greedy_k=2, fixed_k=0)."""
    result = subprocess.run(
        [
            ...
            circuit_name,
            str(timeout),
            "2",              # greedy_k (greedy passes for k=1,2)
            str(n_pool),      # initial_pool_size
            str(n_branch),    # exploration_pool_size
            ...
            ECCSET,
            "0",              # fixed_k=0 → advancing mode (k=3,4,5,...)
        ],
        capture_output=True,
        text=True,
        timeout=timeout + 600,
...
def npool_dispatch(args):
    ...
    print(
        f"  [npool] {circuit_name} npool={n_pool}: {final} gates", flush=True
    )
...
def nbranch_dispatch(args):
    n_branch, filename, circuit_name = args
    pkl_dir = os.path.join(NBRANCH_OUT_DIR, "pkl")
    pkl_path = os.path.join(
        pkl_dir, f"{circuit_name}_nbranch_{n_branch}_{TIMEOUT}.pkl"
    )
    qasm_dir = os.path.join(NBRANCH_OUT_DIR, "qasm")
    qasm_path = os.path.join(
        qasm_dir, f"{circuit_name}_nbranch_{n_branch}.qasm"
    )

    if os.path.exists(pkl_path):
        print(
            f"  [nbranch] {circuit_name} nbranch={n_branch}: cached",
            flush=True,
        )
        with open(pkl_path, "rb") as f:
            return n_branch, circuit_name, pickle.load(f)

    try:
        times, costs, qasm_str = run_qalm(
            filename, circuit_name, TIMEOUT, 1, n_branch
        )
    except Exception:
        traceback.print_exc()
        return n_branch, circuit_name, ([], [])
    ...
    final = int(costs[-1]) if costs else -1
    print(
        f"  [nbranch] {circuit_name} nbranch={n_branch}: {final} gates",
        flush=True,
    )
    return n_branch, circuit_name, (times, costs)
...
def make_aggregate_plots(results, orig_counts, param_name, out_dir):
    """
    results: dict[param_value] -> dict[circuit_name] -> (times, costs)
    Produces 2 PDFs: arith-mean and geomean reduction vs time.
    """
    ...
    ax1.set_title(
        f"{param_name} comparison ({len(plot_circuits)} circuits)"
    )
    ax1.legend(fontsize=9)
    ax1.set_ylim(30, 34)
    fig1.tight_layout()
    path1 = os.path.join(out_dir, f"{param_name}_arith_mean_{TIMEOUT}s.pdf")
    ...
    ax2.set_ylim(30, 34)
    fig2.tight_layout()
    path2 = os.path.join(out_dir, f"{param_name}_geomean_{TIMEOUT}s.pdf")
    fig2.savefig(path2, dpi=150)
    plt.close(fig2)
    print(f"Saved → {path2}")


def print_summary(results, orig_counts, param_name):
    print(f"\n=== {param_name} final gate counts ===")
    for pval in PARAM_VALUES:
        ...
        print(f"  {param_name}={pval}: {', '.join(finals)}")
...
def main():
    ...
    # ── Phase 1: N_pool ──────────────────────────────────────────────────────
    print("=" * 60)
    print("Phase 1: N_pool experiment (N_branch=1 fixed)")
    print("=" * 60)

    for d in [
        NPOOL_OUT_DIR,
        os.path.join(NPOOL_OUT_DIR, "pkl"),
        os.path.join(NPOOL_OUT_DIR, "qasm"),
    ]:
    ...
    print(
        f"Launching {len(npool_tasks)} tasks with {N_WORKERS} workers "
        f"(timeout={TIMEOUT}s, mem_limit=8GB)…"
    )
    with multiprocessing.Pool(N_WORKERS, initializer=_set_mem_limit) as pool:
        npool_raw = pool.map(npool_dispatch, npool_tasks)

    npool_results = {}
    for n_pool, cname, (times, costs) in npool_raw:
        npool_results.setdefault(n_pool, {})[cname] = (times, costs)

    print_summary(npool_results, orig_counts, "N_pool")
    make_aggregate_plots(npool_results, orig_counts, "N_pool", NPOOL_OUT_DIR)

    # ── Phase 2: N_branch ────────────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("Phase 2: N_branch experiment (N_pool=1 fixed)")
    print("=" * 60)

    for d in [
        NBRANCH_OUT_DIR,
        os.path.join(NBRANCH_OUT_DIR, "pkl"),
        os.path.join(NBRANCH_OUT_DIR, "qasm"),
    ]:
        os.makedirs(d, exist_ok=True)

    nbranch_tasks = [
        (n_branch, filename, circuit_name)
        for n_branch in PARAM_VALUES
        for filename, circuit_name in CIRCUIT_LIST
    ]

    print(
        f"Launching {len(nbranch_tasks)} tasks with {N_WORKERS} workers "
        f"(timeout={TIMEOUT}s, mem_limit=8GB)…"
    )
    with multiprocessing.Pool(N_WORKERS, initializer=_set_mem_limit) as pool:
        nbranch_raw = pool.map(nbranch_dispatch, nbranch_tasks)

    nbranch_results = {}
    for n_branch, cname, (times, costs) in nbranch_raw:
        nbranch_results.setdefault(n_branch, {})[cname] = (times, costs)

    print_summary(nbranch_results, orig_counts, "N_branch")
    make_aggregate_plots(
        nbranch_results, orig_counts, "N_branch", NBRANCH_OUT_DIR
    )
