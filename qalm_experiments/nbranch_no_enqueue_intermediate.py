"""
N_branch comparison experiment WITHOUT intermediate enqueue.

Same as the advancing-k nbranch experiment but passes enqueue_intermediate=0,
so only the final circuit (after ROQC) is added to the priority queue.

Run from repo root:
    PYTHONPATH=qalm_experiments python3 qalm_experiments/nbranch_no_enqueue_intermediate.py
"""
...
OUT_DIR = "nbranch_no_enqueue_intermediate_results"
...
def dispatch(args):
    ...
    pkl_path = os.path.join(pkl_dir, f"{circuit_name}_nbranch_{n_branch}_{TIMEOUT}.pkl")
    qasm_dir = os.path.join(OUT_DIR, "qasm")
    qasm_path = os.path.join(qasm_dir, f"{circuit_name}_nbranch_{n_branch}.qasm")

    if os.path.exists(pkl_path):
        print(f"  [nbranch] {circuit_name} nbranch={n_branch}: cached", flush=True)
    ...
    print(f"  [nbranch] {circuit_name} nbranch={n_branch}: {final} gates", flush=True)
...
def make_aggregate_plots(results, orig_counts):
    ...
    for pval, color in zip(PARAM_VALUES, COLORS):
        ...
        if reductions:
            ...
            ax1.plot(common_times, mean_red, label=f"N_branch={pval}",
                     color=color, linewidth=1.8)
    ax1.set_xlabel("Time (s)")
    ax1.set_ylabel("Avg. Gate Count Reduction (%)")
    ax1.set_title(f"N_branch comparison, no intermediate enqueue ({len(plot_circuits)} circuits)")
    ax1.legend(fontsize=9)
    ax1.set_ylim(30, 34)
    fig1.tight_layout()
    path1 = os.path.join(OUT_DIR, f"N_branch_arith_mean_{TIMEOUT}s.pdf")
    fig1.savefig(path1, dpi=150)
    print(f"Saved -> {path1}")
    ax1.set_title(f"N_branch comparison ({len(plot_circuits)} circuits)")
    fig1.tight_layout()
    path1_plain = os.path.join(OUT_DIR, f"N_branch_arith_mean_{TIMEOUT}s_plain.pdf")
    ...
    for pval, color in zip(PARAM_VALUES, COLORS):
        ...
        if log_ratios:
            ...
            ax2.plot(common_times, geomean_red, label=f"N_branch={pval}",
                     color=color, linewidth=1.8)
    ax2.set_xlabel("Time (s)")
    ax2.set_ylabel("Avg. Gate Count Reduction (%)")
    ax2.set_title(f"N_branch comparison, no intermediate enqueue ({len(plot_circuits)} circuits)")
    ax2.legend(fontsize=9)
    ax2.set_ylim(30, 34)
    fig2.tight_layout()
    path2 = os.path.join(OUT_DIR, f"N_branch_geomean_{TIMEOUT}s.pdf")
    fig2.savefig(path2, dpi=150)
    print(f"Saved -> {path2}")
    ax2.set_title(f"N_branch comparison ({len(plot_circuits)} circuits)")
    fig2.tight_layout()
    path2_plain = os.path.join(OUT_DIR, f"N_branch_geomean_{TIMEOUT}s_plain.pdf")
...
def print_summary(results, orig_counts):
    print(f"\n=== N_branch final gate counts (no intermediate enqueue) ===")
    for pval in PARAM_VALUES:
        ...
        print(f"  N_branch={pval}: {', '.join(finals)}")
...
def main():
    ...
    results = {}
    for n_branch, cname, (times, costs) in raw:
        results.setdefault(n_branch, {})[cname] = (times, costs)
